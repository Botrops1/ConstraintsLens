...
13. **Double-click to open edit dialog** — JS dblclick handler + Python `_handle_open_edit_dialog()` framework in place. `_EDIT_DIALOG_COMMANDS` dict has TODO placeholders for the three command strings (SketchOffsetCurvesDimension, CircularPatternConstraint, RectangularPatternConstraint). **Blocked on probe results** from `tests/probe_patterns/probe_patterns.py` (section C) to get the real `executeTextCommand` strings.
...
15. **Editable configurable elements (research + implement)** — **Blocked on probe results** from `tests/probe_patterns/probe_patterns.py` (sections A+B). Probe inspects PolygonConstraint, CircularPatternConstraint, RectangularPatternConstraint object attributes and scans commandDefinitions for edit commands. If writable properties exist, add inline edit; otherwise fall back to dialog open (#13).
