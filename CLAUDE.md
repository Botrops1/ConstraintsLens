...
**Working on:** Maintenance / community issues. v1.6.0–v1.6.3 released and merged to `main`; issues #8/#9/#10/#12 closed. #12 (resizable panes) and the v1.6.3 discoverability pass (high-contrast grips, `?` help sheet) are both **PC-verified 2026-09-03**.
**Version:** 1.6.3 (manifest + commit must always match).
**Next step:** Issue #11 — the palette opens on the primary monitor in a multi-monitor setup. Agreed plan: dock on first creation only (leaving Fusion's remembered state alone on every later run), rather than calling `setPosition`, whose coordinate frame — Fusion-window-relative or desktop-relative — is unverified. Probe before trusting it; guessing wrong puts the palette further off-screen. Note v1.2.0/v1.2.1 already established that forcing dock position over Fusion's own memory is a mistake, so this must be first-run only.
...
- **Unresolved: whether Fusion restores a custom palette's docking state across sessions.** After a restart the throwaway id returned `itemById -> False` and came back Floating at (0,0). But this is NOT conclusive — the reads were all within ~0.4 s of `add()`, and stage 1 called `deleteMe()` on any restored instance before re-adding, so a late restore would have been destroyed unseen. This answer decides the fix: if Fusion restores, the dock must be first-run-only (settings-file gated) so it cannot yank an existing user off their chosen edge; if it does not, the palette floats to (0,0) every session and must be docked on every creation.
