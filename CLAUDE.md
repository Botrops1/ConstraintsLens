...
20. **"Jump to object" button** — Add a button on the left side of each row (next to the constraint icon, same visual size) that scrolls/zooms the canvas to the constraint's entities. Likely implemented via `ui.activeSelections` + Fusion's "Zoom to Selection" text command.
