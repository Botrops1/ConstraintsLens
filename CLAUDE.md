...
**Version:** 1.5.3 (manifest + commit must always match).
**Next step:** PC-verify v1.5.1–v1.5.3 (issues #8, #9, spline guide numbering). For spline guides, run `tests/spike_probe_spline/` first to confirm `controlPointLines` exposes stable entityTokens on the current Fusion build.
...
### Recent fixes (v1.0.1–v1.5.3)
- v1.5.3: Spline guide-line numbering (GitHub spline follow-up). `EntityLabeler` only walked top-level sketch collections, so a control-point spline's guide lines (the control-polygon lines, nested under the spline via `controlPointLines`) had no number and fell back to a bare anonymous "SketchLine" — impossible to tell apart when filtering. New `_index_control_point_splines()` numbers each control-point spline ("Spline N") and each of its guide lines ("Spline N Guide M"), keeping `kind` = SketchLine so icon lookup still resolves. Fully guarded (skips if `controlPointLines`/tokens unavailable). Probe `tests/spike_probe_spline/` added to confirm guide lines expose stable entityTokens on this build before relying on it (pending PC verification). Note: these guide lines are usually `isVisible == False`, so they still render as dimmed "hidden" chips.
- v1.5.2: Issue #9 — palette hide-on-exit / show-on-edit + first-run dock. New `lib/settings.py` (minimal best-effort JSON; first reintroduction since the v1.2.1 `dock_state` removal — different purpose) stores a `first_run_done` flag. `_show_palette()` docks the palette to the right **only** on the very first launch, then defers to Fusion's native dock memory thereafter. `_publish_active()` toggles `palette.isVisible` on sketch enter/exit, acting only on the transition (`_last_in_sketch`) so it never writes `isVisible` on every command; the first publish records state without forcing visibility so it doesn't fight a manual toolbar open. Runtime `settings.json` is gitignored. (pending PC test)
- v1.5.1: Issue #8 — relationship measurements in "Properties of selected". `_build_selection_info()` now collects the raw selected entities and calls `_pairwise_measurement()`, appending a derived "Measurement" footer row (CSS `.sel-item-measure`, accent left-border): 2 points → distance + ΔX/ΔY; 2 lines → acute angle (+ perpendicular gap when parallel); point+line → minimal distance to the segment; 2 circles → offset `|r1−r2|` if concentric else center-to-center; point+circle/arc → distance to center and to edge; 3+ lines → total length. Pure-geometry helpers (`_dist3`, `_angle_between_lines`, `_point_to_segment_dist`, etc.) are best-effort and return None on any failure. (pending PC test)
